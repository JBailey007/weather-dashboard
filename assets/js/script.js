...
const createWeatherCard = (cityName, weatherItem, index) => {
  ...
  if (index === 0) {
    ...
    return (
      ...
      "°F</h4> \
                    <h4>Wind: " +
      (weatherItem.wind.speed * 2.237).toFixed(2) +
      "MPH</h4> \
                    <h4>Humidity: " +
      ...
      '@4x.png" alt="rain cloud with rain below and sun behind"> \
                    <h4>' +
    ...
    return (
      ...
      ')</h3> \
                <img src="https://openweathermap.org/img/wn/' +
      weatherItem.weather[0].icon +
      '@2x.png" alt="rain cloud with rain below and sun behind"> \
                <h4>Temp: ' +
      formattedTemp +
      "°F</h4> \
                <h4>Wind: " +
...
const getCityCoordinates = () => {
  ...
  fetch(GEOCODING_API_URL)
    ...
    .then((data) => {
      if (!data.length)
        return alert("No coordinates fount for " + cityName + "");
...
const saveCityToLocalStorage = (cityName) => {
  ...
  // Add the cityName to the history array
...
const displaySearchHistory = () => {
  ...
  if (history) {
    ...
    // Create and add buttons for each city in the history
